...
function buildRefTexture(): THREE.DataTexture {
  ...
  // Initial positions along the helix (will snap quickly)
  for (let i = 0; i < total; i++) {
    const t = (i % (total / 2)) / (total / 2)
    const strandSign = i < total / 2 ? 1.0 : -1.0
    const helixAngle = t * 8.0 * Math.PI * 2 + strandSign * Math.PI
    data[i * 4 + 0] = Math.cos(helixAngle) * 0.18 * 0.7
    data[i * 4 + 1] = (t - 0.5) * 0.9
    data[i * 4 + 2] = 0.0
    data[i * 4 + 3] = 0.0
  }
