...
void main() {
  ...
  // During hover: orient teardrop along motion direction (fish-like)
  // At rest: orient tangent to ring
  float ringAngle   = atan(vLocalPos.y - uRingPos.y, vLocalPos.x - uRingPos.x);
  float motionAngle = atan(vMotionDir.y, vMotionDir.x);
  float angle = mix(ringAngle, motionAngle, uIsHovering);

  vec2 uv = gl_PointCoord.xy - vec2(0.5);
  uv.y *= -1.0;
  uv = rotate(uv, -angle + (noiseAngle * 0.5));

  // Elongate teardrops slightly more during hover (fish body shape)
  vec2 boxSize = mix(vec2(0.5, 0.2), vec2(0.48, 0.16), uIsHovering);
  ...
  float rounded = sdRoundBox(uv, boxSize, vec4(0.25));
